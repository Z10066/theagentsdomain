...
@Component({
  selector: 'jhi-file-configuration',
  templateUrl: './file-configuration.component.html',
})
export class FileConfigurationComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.fileConfigurationService
      ...
      .subscribe({
        next: (res: HttpResponse<IFileConfiguration[]>) => {
  ...
  trackId(_index: number, item: IFileConfiguration): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: IFileConfiguration[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/file-configuration'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
