...
type RestOf<T extends IVideo | NewVideo> = Omit<T, 'publishedDate'> & {
...
export class VideoService {
  ...
  protected convertDateFromClient<T extends IVideo | NewVideo | PartialUpdateVideo>(video: T): RestOf<T> {
    return {
      ...video,
      publishedDate: video.publishedDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restVideo: RestVideo): IVideo {
    return {
      ...restVideo,
      publishedDate: restVideo.publishedDate ? dayjs(restVideo.publishedDate) : undefined,
    };
  }
