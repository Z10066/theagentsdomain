...
/**
 * Performance test for the Inviteusers entity.
 */
public class InviteusersGatlingTest extends Simulation {
    ...
    String baseURL = Optional.ofNullable(System.getProperty("baseURL")).orElse("http://localhost:9000");
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all inviteusers")
                    .get("/services/loginservice/api/inviteusers")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new inviteusers")
                        .post("/services/loginservice/api/inviteusers")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"invitername\": \"SAMPLE_TEXT\"" +
                                ", \"workspaces\": \"SAMPLE_TEXT\"" +
                                ", \"invitertime\": \"SAMPLE_TEXT\"" +
                                ", \"email\": \"SAMPLE_TEXT\"" +
                                ", \"inviterid\": \"SAMPLE_TEXT\"" +
                                ", \"status\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_inviteusers_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created inviteusers")
                            .get("/services/loginservice${new_inviteusers_url}")
                            .headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created inviteusers")
                        .delete("/services/loginservice${new_inviteusers_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Inviteusers entity").exec(scn);
